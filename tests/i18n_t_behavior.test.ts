...
describe('t(): pending key (untranslated; the dense table English-fills it)', () => {
  ...
  // map; it reads the eager `en` slice plus a `resident.pt_BR` that
  // ensureLocaleLoaded("pt_BR") populates from LOCALE_LOADERS.pt_BR() (a dynamic import
  // of ./pt_BR), plus the static `pending` set. So inject the synthetic pending key at
  // THOSE seams: add it to the pt_BR slice (so resident.pt_BR carries it after the await)
  // and list it in pending.pt_BR (so the release hard-fail fires). Mocking the old barrel
  // `translations` would no longer feed the table the runtime reads - the lookup would
  // miss and throw an UNTRACKED error, a different throw - so the assertion below would
  // silently test the wrong thing.
  const PT_BR = '../src/ui/i18n.resolved.generated/pt_BR';
  ...
  async function loadWithPending() {
    vi.resetModules();
    vi.doMock(PT_BR, async () => {
      const actual =
        await vi.importActual<typeof import('../src/ui/i18n.resolved.generated/pt_BR')>(PT_BR);
      const table = { ...actual.pt_BR, [SAMPLE]: FILL };
      // Expose both `pt_BR` (the source named export) and `default` (the production-chunk
      // shape) so the loader's shape-tolerant `mod.default ?? mod.pt_BR` read resolves
      // without the vitest mock proxy throwing on an undefined `default` access.
      return { pt_BR: table, default: table };
    });
    vi.doMock(PENDING, async () => {
      const actual =
        await vi.importActual<typeof import('../src/ui/i18n.resolved.generated/pending')>(PENDING);
      return {
        pending: { ...actual.pending, pt_BR: [...(actual.pending.pt_BR ?? []), SAMPLE] },
      };
    });
  ...
  it('renders the English fill on a non-release build', async () => {
    ...
    mod.setLanguage('pt_BR');
    await mod.ensureLocaleLoaded('pt_BR'); // make resident.pt_BR carry the synthetic key
  ...
  it('hard-fails on a release build (English must never ship to a translated player)', async () => {
    ...
    const mod = await loadWithPending();
    mod.setLanguage('pt_BR');
    await mod.ensureLocaleLoaded('pt_BR');
...
describe.runIf(RELEASE_TIER)('t(): the committed pending set is empty (release tier)', () => {
  it("every locale's generated pending list is empty", () => {
    ...
    // zero times.
    expect(Object.keys(realPending), 'pending must enumerate the non-en locales').toEqual([
      'pt_BR',
    ]);
...
describe('t(): the empty-pending gate has teeth (a non-empty pending set fails)', () => {
  it('throws when any locale carries a pending key', () => {
    const synthetic: Record<string, readonly string[]> = { pt_BR: ['some.untranslated.key'] };
    expect(() => assertNoPending(synthetic)).toThrow(/pt_BR has unexpected pending keys/);
  });
  it('does not false-positive on a clean all-empty map', () => {
    expect(() => assertNoPending({ pt_BR: [] })).not.toThrow();
  });
});
