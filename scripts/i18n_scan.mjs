...
// override in scripts/i18n_build.mjs. The default keeps the
...
// The three key namespaces. `main` = the resolved-table en leaves; `sim`/`server`
// = the client matcher DICTs (sim_i18n / server_i18n).
...
// Bundle the source locale objects + the matcher DICTs via a
...
async function main() {
  const { en, overlays, serverDICT, simDICT } = await loadSources();
  ...
  // main scope: en leaves. No blocked seed here (cognates live in server).
